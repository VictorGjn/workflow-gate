...
// /approve route, no fingerprint writing, no path to workflow-plan-gate.mjs. The only thing it
// writes is your own notes.
//
...
//
// ponytail: zero dependencies. One JSON cache, one JSON notes file, no database, no index.
...
import { listRuns, costRun, loadCache, saveCache, cachedCost, putCost, PRICES_AT } from './cost.mjs';
...
function summarise(r) {
  ...
  return {
    ...
    resumed: cost ? cost.resumed : 0, unpriced: cost ? cost.unpriced : [],
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname === '/runs' && req.method === 'GET') {
      const runs = listRuns().map(summarise);
      ...
      const all = listRuns();                           // walked once, not once per live run
      for (const r of runs) {
        if (!r.live) { liveState.delete(r.runId); continue; }   // it ended: drop its offsets
        const src = all.find((x) => x.runId === r.runId);
        if (!src) continue;
        const c = await costLive(src);
        r.cost = c.total; r.costUsage = c.usage; r.unpriced = c.unpriced;
      }
      costMissing();                                   // fire and forget; the page polls
      const notes = loadNotes();
      return json({ runs, notes, pricesAt: PRICES_AT, costing, pending: runs.filter((r) => !r.live && r.cost === null).length });
    }
    ...
    if (url.pathname === '/run' && req.method === 'GET') {
      ...
      for (const a of (costed?.agents || [])) {
        ...
        (byLabel[k] = byLabel[k] || { label: k, model: a.model, n: 0, cost: 0, turns: 0, resumed: 0 });
